...
public class MyContactActivity extends StartPanelActivity {
	...
	ArrayList<ListItemDetails> itemMyContact;
	...
	void setupStart() {
		...
		txtFavoriteSellerCount.setText("Contacts (" + 0 + ")");

		itemMyContact = getContactList();
		lvMyContact = (ListView) findViewById(R.id.lvMyContacts);
		lvMyContact
				.setAdapter(new SearchPeopleBaseAdapter(this, itemMyContact));
		lvMyContact.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> a, View v, int position,
					long id) {
				Object o = lvMyContact.getItemAtPosition(position);
	...
	private ArrayList<ListItemDetails> getContactList() {
		...
		resultList = webServiceCall.getUserConnectionList(userID, "");
		...
		if (resultList != null) {
			...
			if (resultList.size() > 0) {
				...
				for (int i = 0; i < resultList.size(); i++) {
					...
					item_details.setTextFooter(getLocationString(
							rowList.get(1), rowList.get(2), rowList.get(3)));
			...
			txtFavoriteSellerCount.setText("Contacts (" + resultsList.size()
					+ ")");
		}
		/*
		 * ListItemDetails item_details = new ListItemDetails();
	...
	static String getLocationString(String strCity, String strState,
			String strCountry) {
		String defaultText = "";
